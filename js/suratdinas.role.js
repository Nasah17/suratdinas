$(window).on('load', function () {
    ...
    // custom toast change role
    function editData() {
        $.NotificationApp.send("Berhasil!", "Data telah diubah.", 'bottom-right', '#5ba035', 'success');
    };

    // inisialisasi APP URL
    const urlWindow = window.location.origin + "/manajemen-menu/role";
    ...
    // var changeRole = document.querySelector('.input.change-role');
    var changeRole
    $('[data-plugin="switchery"]').each(function (idx, obj) {
        changeRole = new Switchery($(this)[0], $(this).data());
    });
    ...
    $(document).on('click', '.change', function () {
        var idRole = $('#modal-role div.modal-body').attr('id-role')
        var idSubmenu = $(this).find('input.change-role').attr('id')
        console.log(idRole + ' ' + idSubmenu)
        $.ajax({
            url: `${window.location.origin}/manajemen-menu/user-access`,
            method: "POST",
            data: `_token=${csrf}&role_id=${idRole}&submenu_id=${idSubmenu}`,
            beforeSend: function () {
                changeRole.disable()
            },
            success: function (data) {
